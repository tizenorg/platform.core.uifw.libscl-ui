...
void Main_Entry_Parser::parsing_main_entry(const char *input_file) {
    xmlDocPtr doc;
    xmlNodePtr cur_node;
    ...
    if (cur_node == NULL) {
        SCLLOG(SclLog::ERROR, "Main_Entry_Parser: empty document.\n");
        xmlFreeDoc(doc);
        exit(1);
    }
    if (0 != xmlStrcmp(cur_node->name, (const xmlChar*)"main-entry"))
    {
       SCLLOG(SclLog::ERROR, "Main_Entry_Parser: root name error: %s\n!", (char *)cur_node->name);
       xmlFreeDoc(doc);
       exit(1);
    }
...
Main_Entry_Parser::get_file_full_path(char* dest_path, const char* file_type) {
    ...
    if (0 == strcmp(file_type, "input_mode_configure")) {
        ...
        SCLLOG(SclLog::ERROR, "file_type : %s is not exist.\n", file_type);
        assert(0);
    }
