...
#ifdef WAYLAND
				Ecore_Wl_Window *wl_dim_window;
				wl_dim_window = elm_win_wl_window_get(static_cast<Evas_Object*>(windows->get_dim_window()));
				if(wl_dim_window)
				{
					if (wl_dim_window->id == ev->window)
					{
						dimwinevent = TRUE;
					}
				}
